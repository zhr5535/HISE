...
void AreaItem::SetNodesSelect(QPointF _p1)
{
	...
	{
		if (scene() == 0) return;
		const auto topView = scene()->views().value(0); 
...
void AreaItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
	...
	QSettings setting;
	auto color =  setting.value("roi_line_color",7/*Qt::red*/).value<QColor>();
	const auto width = setting.value("roi_line_width", 1.0/*0.5*/).toInt();

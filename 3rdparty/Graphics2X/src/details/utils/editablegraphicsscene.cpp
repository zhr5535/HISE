...
#include "actions/ellipsclipper.h"
...
{
	emit cursor_shape(_Mycursor_shape);

	m_last_pressedpos = m_eventpos = e->scenePos();
	if (!in_scene_rect(m_last_pressedpos)) {
	...
	if (e->button() == Qt::RightButton)
	{
		...
		case _MyStatus::READY: {
			// Under this status, do nothing when the right buttion is pressed.
		}
			break;
		...
		case _MyStatus::DRAW_CIRC: {
			auto pCmd = new EllipsPainter(this, m_last_pressedpos);
			pCmd->right_click();
			m_handcursor = false;
			m_isdrawing = false;
			emit complete(m_drawstatus);
		...
		case _MyStatus::DEL_SHAPE:{
			auto pCmd = new DeleteAreaCommand(this, m_last_pressedpos);
			pCmd->exec();
			break; 
		}
		}

		if (m_drawstatus == _MyStatus::READY) {
			m_drawstatus = m_drawstatus_before_rightclick;
		}
		else {
			m_drawstatus_before_rightclick = m_drawstatus;
	...
	if (e->button() == Qt::LeftButton)
	{
		...
		case _MyStatus::READY:
			if (!m_isdrawing){
				auto pCmd = new HandCursorEvent(this, m_eventpos);
				pCmd->press();
				m_isdrawing = false;
				emit complete(m_drawstatus);
			}
			break;
		case _MyStatus::DRAW_LINE:
			if (!m_isdrawing) {
				m_last_pressedpos = e->scenePos();
				m_handcursor = false;
				m_isdrawing = true;
				emit complete(m_drawstatus);
			}
		...
		case _MyStatus::DRAW_RECT:
			...
			{
				ShapeEvent* pCmd = nullptr;
				pCmd = new RectPainter(this, m_last_pressedpos, m_last_pressedpos);
				pCmd->exec();
				m_handcursor = false;
				m_isdrawing = true;
			}
			...
			{
				ShapeEvent* pCmd = nullptr;
				pCmd = new RectPainter(this, QPointF(0, 0), QPointF(0, 0));
				pCmd->merge();
				m_handcursor = true;
				m_isdrawing = false;
				emit complete(m_drawstatus);
		...
		case _MyStatus::DRAW_POLY:
			if (!m_isdrawing) { // start drawing polygon
				auto _Cmd = new PolygonPainter(this, m_last_pressedpos);
				_Cmd->exec();
				m_handcursor = false;
				m_isdrawing = true;
			}
		...
		case _MyStatus::DRAW_CIRC:
			...
			{
				auto _Cmd = new EllipsPainter(this, m_last_pressedpos);
				_Cmd->exec();
				m_handcursor = m_isdrawing = true;
			}
			else{
				ShapeEvent* pCmd = nullptr;
				pCmd = new EllipsPainter(this, m_last_pressedpos);
				pCmd->merge();
				if (!m_isdrawing) emit complete(m_drawstatus);
			}
			break;
		case _MyStatus::MOVE_REGION:
			{ // start to move selected region
				for (auto it = m_shapes.cbegin(); it != m_shapes.cend(); ++it) {
					if (IsInRegion(m_eventpos.x(), m_eventpos.y(), *it)) {
						m_selectedregions = it - m_shapes.cbegin();
						(new HandCursorEvent(this, m_eventpos))->press();
					}
				}
				m_handcursor = false;
				m_isdrawing = true;
		...
		case _MyStatus::DEL_SHAPE: // delete area node or area
			// if delete one node or a seed, we'll not delete the area
			if (!DeleteSeed(m_eventpos) && !DeleteNode(m_last_pressedpos)) {
				auto pCmd = new DeleteAreaCommand(this, m_last_pressedpos);
				pCmd->exec();
				emit delete_item(2);
			}
		...
		case _MyStatus::CLIP_RECT:
			if (!m_shapes.isEmpty()) {
				...
				{
					ShapeClippingEvents* pCmd = new RectClipper(this, m_last_pressedpos, m_last_pressedpos);
					pCmd->exec();
					m_handcursor = false;
					m_isdrawing = true;
					m_is_clipping = true;
		...
		case _MyStatus::CLIP_POLY:
			...
			{
				ShapeClippingEvents* pCmd = new PolygonClipper(this, m_last_pressedpos);
				pCmd->exec();
				m_handcursor = false;
				m_isdrawing = true;
				m_is_clipping = true;
			...
			{
				auto pCmd = new PolygonClipper(this, m_last_pressedpos);
				pCmd->next();
			}
			break;
		case _MyStatus::CLIP_CIRC:
			...
			{
				ShapeClippingEvents* pCmd = new EllipsClipper(this, m_last_pressedpos);
				pCmd->exec();
				m_handcursor = false;
				m_isdrawing = true;
				return;
			...
			break;
		case _MyStatus::DRAW_POINT:  // add new scatter point
		{
			auto point = new PointItem(PointItem::type_t(m_ptstyle));
			addItem(point);
			point->setPos(m_last_pressedpos);
			point->set_label(m_points.empty() ? 1 : m_points.back()->label()+1);
			m_points << point;
			m_isdrawing = false;
			emit complete(m_drawstatus);
		} break;
		...
		{
			emit SignalSSSIGPointPicked(QPointF(m_last_pressedpos));
...
void EditableGraphicsScene::mouseMoveEvent(QGrSME *e)
{
	...
	emit cursor_shape(sceneRect().contains(mouse_pos) ? _Mycursor_shape : Qt::OpenHandCursor);

	if (!m_isdrawing && m_drawstatus == _MyStatus::READY) {
		auto pCmd = new HandCursorEvent(this, mouse_pos);
		...
		if (m_is_seed_selected && m_translate_seed != nullptr) {
			auto pos = CorrectPointInSceneRect(m_last_seed_pos + e->scenePos() - m_last_pressedpos);
			m_translate_seed->setPos(pos);   // the the seed position
			move_cursor_label(mouse_pos);  // move the coordinate label
			return;
		}
	...
	if (m_isdrawing){
		...
		{
		case _MyStatus::READY:{
			m_isdrawing = false; break;
		}
		case _MyStatus::DRAW_LINE: {
			auto _Item = new QGraphicsLineItem();
			_Item->setPen(QPen(Qt::green, 3, Qt::SolidLine));
			_Item->setPos(m_last_pressedpos);
			const auto _Diff = mouse_pos - m_last_pressedpos;
			_Item->setLine(0, 0, _Diff.x(), _Diff.y());
			this->addItem(_Item);
		} break;
		case _MyStatus::DRAW_RECT:{
			auto pCmd = new RectPainter(this, m_last_pressedpos, mouse_pos);
			pCmd->move();
		} break;
		case _MyStatus::DRAW_CIRC: {
			ShapeEvent* pCmd = new EllipsPainter(this, mouse_pos);
			pCmd->move();
		} break;
		case _MyStatus::DRAW_POLY: {
			auto _Cmd = new PolygonPainter(this, mouse_pos);
			_Cmd->move();
		} break;
		case _MyStatus::CLIP_RECT:{
			ShapeClippingEvents* pCmd = new RectClipper(this, m_last_pressedpos, mouse_pos);
			pCmd->move();
		} break;
		case _MyStatus::CLIP_POLY:{
			ShapeClippingEvents* pCmd = new PolygonClipper(this, mouse_pos);
			pCmd->move();
		} break;
		case _MyStatus::CLIP_CIRC:{
			ShapeClippingEvents* pCmd = new EllipsClipper(this, mouse_pos);
			pCmd->move();
		} break;
		case _MyStatus::MOVE_REGION:{ // Move only when the left button is pressed
			if (e->buttons() == Qt::LeftButton) {
				const auto _Off = mouse_pos - m_last_pressedpos;
				if (!m_shapes.empty()) {
					auto _Region = m_shapes[m_selectedregions];
					if (!IsInRegion(mouse_pos.x(), mouse_pos.y(), _Region)) break;
					_Region->setPos(_Region->pos() + _Off);
					for (auto i = 0; i < _Region->node_list.size(); ++i) {
						decltype(auto) _Node = _Region->node_list[i];
						_Node->setPos(_Node->pos() + _Off);
					}
...
void EditableGraphicsScene::mouseReleaseEvent(QGrSME* e)
{
	...
	for (int _Idx = 0; _Idx < m_shapes.length(); _Idx++)
	{
		if ((m_shapes.value(_Idx)->item_select)&&(m_drawstatus==_MyStatus::READY)) {
			m_selectedregions = _Idx;
...
{
	if ((m_translate_seed = 
		cursor_captured_point(e->scenePos())) != nullptr) 
	{
...
bool EditableGraphicsScene::DeleteSeed(QPointF pos)
{
	for (int i = 0; i < m_points.length(); i++) {
		double scalfac_previous = views().value(0)->transform().m11();
		const auto point = QPointF(scalfac_previous*(pos.x() - m_points.value(i)->x()), 
			scalfac_previous*(pos.y() - m_points.value(i)->y()));
		if (m_points.value(i)->mutable_shape()->contains(point)) {
			removeItem(m_points.value(i));
			m_points.removeAt(i);
			m_isdrawing = false;
			emit complete(status_type::DEL_NODE);
			emit deleted_point_item(i);
			emit delete_item(1);
			return true;
		}
	}
...
bool EditableGraphicsScene::DeleteNode(QPointF point)
{
	...
	{
		if (area->item_select) {
			const auto count = area->node_list.length();
			auto scalfac = this->views().first()->transform().m11(); // scale factor of the view
			foreach(NodeItem * node, area->node_list)
			{
				if (node->boundingRect().contains(node->mapFromScene(point) * scalfac)) // if point in node
				{
					if (count > 3) {
						auto areas = AreaDeleteNodeToAreas(area, node);
...
{
	if (m_points.length()<=0) return QPointF(-1,-1);

	SeedItem* start_point = m_points.last();
...
{ 
	...
	auto _area = m_shapes.last();
	DelArea(_area);
	m_shapes.removeOne(_area);

	auto area_circle = new ShapeItem();
	addItem(area_circle);
	...
	qreal deltaAlpha = 2 * pi_v<qreal> / edges;
...
{ 
	...
	for (auto i=0.0; i< 2*pi_v<qreal>; i += 2*pi_v<qreal> /edges)
	{
		...
		if (count == 0){
			auto node = new NodeItem(area_circle);
...
void EditableGraphicsScene::GetVertDeuceLine(QPointF pt1, QPointF pt2, _Equation_params &para)
{ 
	QPoint   cPt; 
...
bool EditableGraphicsScene::GetIntersectionPoint(_Equation_params para1, 
	_Equation_params para2, QPointF &center_point) 
{ 
			...
			while(q != 0)
			{
				...
				if (ptn > 1)
				{
					if (QString::compare(markerList[0],markerList[ptn-1]) == 0)//首末点点名相同时裁剪出一个区域
					{
						...
						pointResultList << QPointF(-1.0,-1.0);
						clipResuAreaNum++;
						q = 0;
					}
				}
...
bool EditableGraphicsScene::IsInRegion(double x, double y, AreaItem* area )
...
/* 应该是移动鼠标的时候添加区域 */
{
		...
		area_order++)//遍历比较m_shapes中所有区域
	{
		...
		PolygonMerger mergeCmd(this, m_shapes.value(area_order), _area);
		mergeCmd.MergeMove();
...
void EditableGraphicsScene::ClipInitSet()//剪裁
{
	...
	for (decltype(auto) shape : m_shapes) {
		shape->setVisible(true);
	}
	//clear
	if (m_temp_shapes.isEmpty()) return;
	for(int i = 0;i<m_temp_shapes.count();i++) {
		for (int k = 0; k < m_temp_shapes[i]->node_list.length(); k++) {
...
QList<Rgn2D> EditableGraphicsScene::get_areas() const noexcept
{
	QList<QList<QPointF>> pListArea;
	for (int i = 0; i < m_shapes.length(); ++i) {
		QList<QPointF> qlist;
		decltype(auto) _area = m_shapes.value(i);
...
{
	clear_all_points();
	for (const auto& p : _pList) {
		DrawExternalStartPoint(p);
	}
}

void EditableGraphicsScene::clear_all_points()
{
	while(m_points.length()) {
		if (m_points.last()) {
			removeItem(m_points.last());
			m_points.removeLast();
		}
	}
}
...
/* 将所有种子点的几何信息存储在一个 QList<QPointF> 里面 */
{
	...
	{
		QPointF _point;
		_point.setX(m_points.value(i)->x());
		_point.setY(m_points.value(i)->y());
		_plist << _point;
	}
	return _plist;
}
...
auto EditableGraphicsScene::_Get_roi_corners(QPointF point, std::initializer_list<double> cn)const noexcept {
...
{
	auto leftx = width(), lefty = height(), rightx = 0., righty = 0.;

	//for (auto i = 0; i < m_shapes.size(); ++i)
	for(const auto& shape : m_shapes) {
		for(int j = 0; j < shape->GetPolygon().size(); ++j) {
			auto point= shape->GetPolygon().value(j);
...
bool EditableGraphicsScene::empty() const noexcept {
	return m_shapes.isEmpty() && m_points.isEmpty();
}
...
void EditableGraphicsScene::move_point(size_t idx, float_t dx, float_t dy) noexcept {
	m_points[idx]->moveBy(qreal(dx), qreal(dy));
}
void EditableGraphicsScene::move_point_to(size_t idx, float_t x, float_t y) noexcept {
	m_points[idx]->setPos({ x, y });
}

/* the pos in which seed */
PointItem* EditableGraphicsScene::cursor_captured_point(QPointF pos) {
	const auto scalfac = views().value(0)->transform().m11();
	for(decltype(auto) pt : m_points) {
		decltype(auto) posInItem = pt->mapFromScene(pos);
		if (pt->shape().contains(posInItem*scalfac))
			return pt;
	}
	return nullptr;
}
...
NodeItem* EditableGraphicsScene::cursor_captured_node(QPointF pos)
{
	...
	const auto scale_factor=trans.m11();
	foreach(ShapeItem* area, m_shapes) {
		foreach (NodeItem* node, area->node_list) {
			const auto posi = node->mapFromScene(pos);
			if (node->boundingRect().contains(posi * scale_factor)) return node;
		}
	}
...
void EditableGraphicsScene::draw_free_point(const QPainterPath& shape) noexcept
{
	m_drawstatus = _MyStatus::DRAW_POINT;
	
	//next, it is need to pass the shape to the point item
}

void EditableGraphicsScene::draw_single_line(QPointF start, QPointF end) noexcept
{
	addLine(start.x(), start.y(), end.x(), end.y(), Qt::SolidLine);
}

bool EditableGraphicsScene::in_scene_rect(QPointF pos) noexcept
{
...
void EditableGraphicsScene::clear_all_regions() noexcept
{
...
QGraphicsPixmapItem* EditableGraphicsScene::set_pixmap(QImage* ptr) noexcept
{
	return addPixmap(QPixmap::fromImage(*ptr));
}

const EditableGraphicsScene::pointitem_list& EditableGraphicsScene::get_points() const noexcept
{
	return m_points;
}
EditableGraphicsScene::pointitem_list& EditableGraphicsScene::get_points() noexcept
{
	return m_points;
}
...
// if the pos is in a seed return 4;
{
	if (m_isdrawing)
		...
		return 0;
	else if (cursor_captured_point(pos) != nullptr)
		return 4;
...
QSet<PointItem*> EditableGraphicsScene::seedInArea(ShapeItem* area)
{
	QSet<PointItem*> seedInAreaList;
	for(decltype(auto) seed:m_points) {
		const auto seedPosition = seed->scenePos();
...
void EditableGraphicsScene::add_cursor_label()
{
	QSettings settings;
	...
	m_horcursor_info =new CursorLabel("");
	m_vercursor_info =new CursorLabel("");
...
void EditableGraphicsScene::delete_cursor_label()
{
...
void EditableGraphicsScene::move_cursor_label(QPointF pos)
{
	const auto trans = views().value(0)->transform();
	const auto scalfac = trans.m11();

	if (m_is_cursor_info_visible) {
		auto rect = sceneRect();
		auto x_start = pos + QPointF(10, 10) / scalfac;
		...
		const auto y_start = m_horcursor_info->scenePos() + 
			QPointF(m_horcursor_info->boundingRect().width()+3,0)/scalfac;
		m_vercursor_info->setPos(y_start);
		m_vercursor_info->setPlainText(QString(" y : %1 ").arg(QString::
			number(pos.y(),'f',2)));
...
void EditableGraphicsScene::ClearLastArea()
{
	if (m_isdrawing){
		...
		m_shapes.removeOne(m_shapes.last());
		m_isdrawing = false;
		emit complete(status_type::DEL_SHAPE);
	}
}
...
QList<QPointF> EditableGraphicsScene::AreaToPointList(ShapeItem* area)
{
	...
	for(auto node : area->node_list) /* for each node of area */
		pointList << node->scenePos();
...
QList<QPolygonF*> EditableGraphicsScene::PListToPolygons(QList<QPointF> pointList)
{
	...
	if (!pointList.isEmpty()) {
		for (auto i = 0; i < pointList.length(); i++)
		{
...
void EditableGraphicsScene::PolygonToCorrectOrder(QList<QPolygonF*>& polygons)
{
	...
	for (int i = polygons.count()-1; i > 0; i--)
	{
		...
		if (!isContain) index = i-1;
	}
	if (index > 0)
#if QT_VERSION < QT_VERSION_CHECK(6,0,0)
		polygons.swap(0, index);
#else
		polygons.swapItemsAt(0, index);
#endif
}
